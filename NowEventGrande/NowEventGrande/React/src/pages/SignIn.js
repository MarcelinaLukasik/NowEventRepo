...
function SignIn() {
    ...
    const [loggedUser, setLoggedUser] = useState(localStorage.getItem("isLogged"));
        ...
        <div>
            ...
            <Modal.Header >           
              ...
              <form method="post">
                ...
                <div>
                    <input asp-for="Input.UserName" className="registerInput" autoComplete="username" aria-required="true"
                     placeholder="UserName" value={userName} onChange={(evt) => {setUserName(evt.target.value)}}/>
                    <label asp-for="Input.Email"></label>
                    <span asp-validation-for="Input.Email" className="text-danger"></span>
                </div>
                <div className='passwordInput'>
                    <input asp-for="Input.Password" className="registerInput" autoComplete="new-password" 
                    aria-required="true" placeholder="Password" value={password} onChange={(evt) => {setPassword(evt.target.value)}}
                    type={passwordType}/>              
                     <button className="visibilityButton" onClick={togglePasswordVisibility}>
                     { passwordType==="password"? <EyeSlash size={24}/> :<Eye size={24}/> }
                     </button>
                ...
                {!valid && <div className="wrongInputMessage">Invalid credentials.</div>}
            </form>
              </Modal.Body>
          </Modal>
        </div>
