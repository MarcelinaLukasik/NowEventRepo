...
function Offer() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      res.json().then((res) => setStatus(res));
    }
    fetchData();
  }, []);
      ...
      <div>
        ...
        <div className="row">
          ...
          <div className="Event-col-6">
            {status && (
              <div>
                <h3 className="offerInformation">
                  Good job, your event checklist is now completed. Make sure all
                  data is correct before you post your offer.
                </h3>
                <button
                  type="submit"
                  className="postOffer"
            ...
            {!status && (
              <h3 className="offerInformation">
                Your event is not completed yet. Provide all required
          ...
          {statusMsg.message && (
            <Col>
              <p className={statusMsg.success === false ? "danger" : "success"}>
                {statusMsg.message}
              </p>
            </Col>
          )}
